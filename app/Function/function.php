...
function renderLinkAddPostType()
{
    ...
    if ($type == 'blog') {
        return [
            'title'    => 'Bài Viết',
            'linkAdd'  => route('post.create', ['type' => 'blog']),
            'linkList' => route('post.index', ['type' => 'blog']),
        ];
    } else {
        return [
            'title'    => 'Tuyển dụng',
            'linkAdd'  => route('post.create', ['type' => 'recruitment']),
            'linkList' => route('post.index', ['type' => 'recruitment']),
        ];
    }
}
...
{
    if(!empty($key)){
        $data = Setting::where('type', $key)->first();
        if(!empty($data)){
