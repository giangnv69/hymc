                    ...
                    @foreach ($data as $item)
                        @if (empty($item->parent_id))
                            <li class="dd-item" data-id="{{ $item->id }}">
                                <div class="dd-handle">
                                    {{ $item->title_vi }} - {{ $item->title_en }} (<i>{{ $item->url }}</i>)
                                </div>
                ...
                <form action="{{ route('setting.menu.addItem', $id ) }}" method="POST" class="frm_add">
                    <div class="modal-body">
                        ...
                        <fieldset class="form-group">
                            ...
                            <input type="text" class="form-control" placeholder="Nhập tiêu đề" name="title_vi" required>
                        </fieldset>
                        <fieldset class="form-group">
            ...
            <div class="modal-content">
                ...
                <form action="{{ route('setting.menu.editItem' ) }}" method="POST" class="frm_add">
                    <div class="modal-body">
                        ...
                        <fieldset class="form-group">
                            <label>Tiêu đề Tiếng Việt</label>
                            <input type="text" class="form-control" id="editTitle" name="title_vi" required >
                            <input type="hidden" value="" id="id_menu" name="id">
                        </fieldset>
                        <fieldset class="form-group">
                            <label>Tiêu đề Tiếng Anh</label>
                            <input type="text" class="form-control" id="editTitle2" placeholder="Nhập tiêu đề" name="title_en" required>
                        </fieldset>
                        <fieldset class="form-group">
                            <label>Đường đẫn</label>
        ...
        $('.modalEditMenu').click(function(event) {
            ...
            $.get('{{ asset('/backend/menu/edit-item/') }}/'+id, function(data) {
                if(data.status == "success"){
                    $('#editTitle').val(data.data.title_vi);
                    $('#editTitle2').val(data.data.title_en);
